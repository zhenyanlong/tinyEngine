...
- **保持文件现有的所有历史记录不变**
- **⚠️ 拼写安全警告**：禁止在 Write 调用中只写入新几天的内容。Write 的内容必须包含**完整文件全部历史记录**（新日期 + 所有旧日期 + 旧内容），否则 Write 工具会覆写文件，导致所有未包含的历史记录永久丢失。**正确的做法**：Read 出完整文件内容 → 拼接新日期到最前面 → Write 写入完整拼接结果。**错误的做法**：自己构造只包含新日期的内容直接 Write，会删除所有旧历史。
...
- **⚠️ 拼写安全警告**：同步骤 3，禁止只写新内容，必须包含完整文件全部历史记录
